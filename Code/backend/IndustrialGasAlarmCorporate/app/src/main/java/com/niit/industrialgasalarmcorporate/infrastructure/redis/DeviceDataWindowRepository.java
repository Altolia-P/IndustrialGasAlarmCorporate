...
public class DeviceDataWindowRepository {
    ...
    public void addDataPoint(String deviceUuid, double concentration, long timestampSeconds) {
        String key = KEY_PREFIX + deviceUuid;
    ...
    public long countExceededInWindow(String deviceUuid, long windowStartSeconds) {
        String key = KEY_PREFIX + deviceUuid;
        Long count = stringRedisTemplate.opsForZSet().count(key, windowStartSeconds, Double.POSITIVE_INFINITY);
        return count != null ? count : 0;
    }

    public void removeExpired(String deviceUuid, long beforeSeconds) {
        String key = KEY_PREFIX + deviceUuid;
